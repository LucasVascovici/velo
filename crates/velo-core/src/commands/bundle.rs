...
impl Applied {
    /// The bundle held nothing this repository didn't already have.
    pub fn is_empty(&self) -> bool {
        self.snapshots == 0 && self.objects == 0
    }
}
...
pub fn create(repo: &Repo, file: &str, target: Option<&str>) -> Result<Created> {
    ...
    let snap_set: HashSet<String> = match target {
        Some(t) => {
            let tip = crate::commands::resolve_snapshot_id(repo, t)?;
    ...
    if snap_set.is_empty() {
        return Err(VeloError::invalid(
            "Nothing to bundle — the repository has no snapshots.",
        ));
    }
...
pub(crate) fn import_pack(
    ...
    let existing: HashSet<String> = {
        let mut stmt = guard.conn().prepare("SELECT hash FROM snapshots")?;
        let set = stmt
            .query_map([], |r| r.get::<_, String>(0))?
            .filter_map(|r| r.ok())
            .collect();
...
impl<'a> Reader<'a> {
    ...
    fn string(&mut self) -> Result<String> {
        ...
        String::from_utf8(b.to_vec())
            .map_err(|_| VeloError::invalid("Bundle contains invalid UTF-8."))
    }
}

fn truncated() -> VeloError {
    VeloError::invalid("Bundle is truncated or corrupt.")
}

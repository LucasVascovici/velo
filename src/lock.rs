...
impl RepoLock {
    /// Acquire the exclusive repo lock, failing fast if another `velo` process
    /// already holds it (rather than blocking indefinitely).
    pub fn acquire(root: &Path) -> Result<Self> {
        ...
        match file.try_lock_exclusive() {
            ...
            Err(_) => Err(VeloError::InvalidInput(
